...
|---|---|---|---|
| `E-20260926-GPU1793-TMPDIR` | IS-A→INT06 GPU1793 exited before model startup because Slurm set `TMPDIR=/tmp`, outside the permitted write boundary | No ML path or force result; input/checkpoint preflight PASS did not cover the scheduled environment | Explicitly export `/home/sbq/sbq/aqcat25/tmp`; no-model setup passes. Preserve failed receipt; obtain one environment-fixed resubmission authorization. See `docs/reviews/is_a_int06_gpu1793_bootstrap_failure_20260926.md` |
