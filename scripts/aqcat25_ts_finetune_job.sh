...
aqcat25_require_remote_path "$TRAINING_MANIFEST" || exit 2

aqcat25_setup_mz73_environment "$RUN_ROOT" "$OUTPUT_ROOT" || exit 2
...
run_training() {
  ...
  aqcat25_require_remote_path "$BASE_CHECKPOINT" || return 2
  aqcat25_require_sha256 "$BASE_SHA" || return 2
